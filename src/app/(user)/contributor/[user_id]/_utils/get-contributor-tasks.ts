...
interface GetContributorTasksParams {
  organization: {
    id: number
  }
...
export type ContributorTasks = PaginatedCollection<ContributorTask> & {
...
export async function getContributorTasks(
  params: GetContributorTasksParams,
): Promise<ContributorTasks> {
  const {
    organization,
    contributor_id,
    category = 'all',
  ...
  const baseUrl = `contributor/${contributor_id}`
