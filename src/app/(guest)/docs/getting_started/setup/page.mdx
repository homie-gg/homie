...
Select the repositories you'd like to track, or just track everything by leaving
it as 'All repositories':
...
Select Gitlab by clicking the Install Gitlab button. You'll then be asked to
authorize Homie to use your account:
...
To track projects, go into **Settings > Gitlab**, and select the projects that
you'd like Homie to track.
...
Slack is where you'll chat with Homie. You can send messages, receive reports,
and trigger manual actions. To do all that, we need to install the Homie app to
...
Then, select your workspace on the top-right, as well as a Slack channel that
Homie will be able to messages to.
...
<Alert>
  Homie can reply to messages in _any_ channel, but can only initiate messages
  in the channel selected above.
</Alert>
